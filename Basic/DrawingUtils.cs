...
namespace Basic
{
    public class DrawingUtils : ClassItem
    {
        ...
        public static NXOpen.Drawings.DrawingSheet DrawingSheet(string sheetPath, double height, double length, string drawName)
        {
            ...
            draftingDrawingSheetBuilder1.AutoStartViewCreation = true; //自动创建视图设置
            draftingDrawingSheetBuilder1.StandardMetricScale = NXOpen.Drawings.DrawingSheetBuilder.SheetStandardMetricScale.S11;     //设置图纸比例为1:1
            draftingDrawingSheetBuilder1.Height = height;
            draftingDrawingSheetBuilder1.Length = length;
            draftingDrawingSheetBuilder1.ScaleNumerator = 1.0; //自定义比例分子
            draftingDrawingSheetBuilder1.ScaleDenominator = 1.0; //自定义比例分母
            draftingDrawingSheetBuilder1.Units = NXOpen.Drawings.DrawingSheetBuilder.SheetUnits.Metric; //图纸单位为公制
            draftingDrawingSheetBuilder1.ProjectionAngle = NXOpen.Drawings.DrawingSheetBuilder.SheetProjectionAngle.Third; //投影视角为第三视角
            draftingDrawingSheetBuilder1.Number = "1"; //创建或编辑图纸的编号
            draftingDrawingSheetBuilder1.SecondaryNumber = ""; //创建或编辑图纸的次要编号
            draftingDrawingSheetBuilder1.Revision = "A"; //创建或编辑图纸的修订版
            draftingDrawingSheetBuilder1.MetricSheetTemplateLocation = sheetPath;   //模板地址
            draftingDrawingSheetBuilder1.Name = drawName;
            try
            {
        ...
        public static Tag CreateTable(string templatePath, double[] origin, int rows)
        {
            ...
            {
                LogMgr.WriteLog("DrawingUtils.CreateTable" + ex.Message);
                throw ex;
            }
        }
