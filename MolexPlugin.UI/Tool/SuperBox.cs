...
namespace MolexPlugin
{
    ...
    public partial class SuperBox
    {
        ...
        {
            if (superBox != null && superBox.ToolingBoxFeature != null) //边界盒没高度
            {
                if (selectedObject is Face)
                {
                    Face face = selectedObject as Face;
                    if (face.GetBody().Tag == superBox.ToolingBoxFeature.GetBodies()[0].Tag)
                        return UFConstants.UF_UI_SEL_REJECT;
                }
                if (selectedObject is Edge)
                {
                    Edge edge = selectedObject as Edge;
                    if (edge.GetBody().Tag == superBox.ToolingBoxFeature.GetBodies()[0].Tag)
                        return UFConstants.UF_UI_SEL_REJECT;
                }
                if (selectedObject is Point)
                {
                    ...
                    for (int i = 0; i < parents; i++)
                    {
                        ...
                        if (type == UFConstants.UF_solid_type)
                        {
                            ...
                            if (bodyTag == superBox.ToolingBoxFeature.GetBodies()[0].Tag)
                                return UFConstants.UF_UI_SEL_REJECT;
                        }
                    }
                }
