...
namespace MolexPlugin.DAL
{
    ...
    /// 直加等宽
    /// </summary>
    public class PlanarAndSufaceEleTemplate : AbstractElectrodeTemplate
    {
        public PlanarAndSufaceEleTemplate(CompterToolName tool) : base(tool)
        {
            this.Type = ElectrodeTemplate.PlanarAndSufaceEleTemplate;
        }
        ...
        public override void CreateProgramName()
        {
            ...
            SurfaceContourCreateOperation so = new SurfaceContourCreateOperation(count, tool.GetFinishBallTool()); //等宽
            so.CreateOperationName(1);
            so.SetFlat(false, false);
            Programs.Add(new ProgramOperationName(count, so));
            count++;
